        ...
        while not stop_event.is_set():
            try:
                if active_stream_config and not stream_fallback_to_tcp:
                    ...
                    if active_http_poll_interval_s is not None and active_http_poll_interval_s > 0:
                        ...
                        while True:
                            ...
                            conn = conn_cls(poll_target["host"], poll_target["port"], timeout=30)
                            try:
                                conn.request(
                                    "GET",
                                    poll_target["path"],
                        ...
                        try:
                            while True:
                                ...
                                conn = conn_cls(stream_target["host"], stream_target["port"], timeout=30)
                                conn.request(
                                    "GET",
                                    stream_target["path"],
                            ...
                            while not stop_event.is_set():
                                try:
                                    payload = response.read(max(64, int(stream_read_chunk_bytes)))
                ...
                if (
                    ...
                    fallback_text = f"⚠️  [RTCM] stream endpoint unstable; falling back to TCP {host}:{port}"
                    print(fallback_text)
                    write_log(fallback_text)
...
def main():
    ...
    parser.add_argument("--silence-timeout", type=float, default=8.0, help="Warn if no NAV-PVT is seen for this many seconds")
    parser.add_argument("--rtcm-enable", dest="rtcm_enable", action="store_true", default=True, help="Enable RTCM forwarding from base endpoint to ZED-F9P")
    parser.add_argument("--no-rtcm-enable", dest="rtcm_enable", action="store_false", help="Disable RTCM forwarding from base endpoint")
    parser.add_argument(
        "--rtcm-stream-url",
        default=os.environ.get("RTCM_STREAM_URL", "http://192.168.4.1:8080/rtcm/stream").strip() or "http://192.168.4.1:8080/rtcm/stream",
        help="HTTP/HTTPS RTCM endpoint (e.g. http://192.168.4.1:8080/rtcm/stream)",
    )
    parser.add_argument("--rtcm-host", default=os.environ.get("RTCM_BASE_HOST", "192.168.4.1"), help="RTCM TCP host")
    parser.add_argument("--rtcm-port", type=int, default=int(os.environ.get("RTCM_BASE_PORT", "2101")), help="RTCM TCP port")
    parser.add_argument("--rtcm-http-poll-interval", type=float, default=None, help="For HTTP endpoint, poll interval seconds")
    parser.add_argument("--rtcm-stream-fallback-errors", type=int, default=3, help="Consecutive stream errors before fallback to TCP host:port")
    parser.add_argument("--rtcm-stream-read-chunk", type=int, default=256, help="HTTP stream read chunk size in bytes")
    parser.add_argument("--show-nmea", action="store_true", help="Parse and print NMEA messages")
    parser.add_argument(
        "--rtcm-wifi-ssid",
        default=os.environ.get("RTCM_WIFI_SSID", "").strip() or os.environ.get("BASE_WIFI_SSID", "").strip() or "CHANGE_ME_SSID",
    ...
    parser.add_argument("--rtcm-log-file", default=os.environ.get("RTCM_LOG_FILE", "rtcm_log.txt").strip(), help="Text file path for RTCM logs")
    ...
    while True:
        ...
        with ser:
            ...
            rtcm_thread = None
            if args.rtcm_enable:
                rtcm_target = args.rtcm_stream_url or f"{args.rtcm_host}:{args.rtcm_port}"
            ...
            try:
                for _, msg in ubr:
                    ...
                    if msg.identity == "NAV-PVT":
                        last_nav_pvt_time = time.time()
                        fix_str = fix_type_to_str(msg.fixType)
                        rtk_str = rtk_status_to_str(msg.carrSoln)
                        num_sv = msg.numSV
                        lat = msg.lat * 1e-7
                        lon = msg.lon * 1e-7
                        height_m = msg.hMSL / 1000.0
                        nav_text = (
                            f"[NAV-PVT] Fix: {fix_str:12} | [RTK] {rtk_str:10} | "
                            f"Sats: {num_sv:2} | Lat/Lon: {lat:.7f},{lon:.7f} | Alt: {height_m:.2f} m"
                        )
                    ...
                    if now - last_nav_pvt_time > args.silence_timeout:
                        log_step("⚠️  No NAV-PVT recently", args.rtcm_log_file)
